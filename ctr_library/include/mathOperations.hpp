...
{
/**
 * @brief Enumeration for defining which root finding algorithm to be used by the shooting method to solve the CTR's
...
/**
 * @brief Computes and returns the least quaternion rotation between two user provided vectors vectors a and b. The
 * rotation is from vector a to vector b. Both vectors are 3-dimensional.
 *
 ...
 */
inline blaze::StaticVector<double, 4UL> getRotationBetween(const blaze::StaticVector<double, 3UL> &a,
...
/**
 * @brief Converts a rotation matrix in SO(3) into the corresponding orientation in quaternions. The quaternion is
 * passed by reference and is modified by the function.
 *
 ...
 */
inline void getSO3(const blaze::StaticVector<double, 4UL> &h,
...
inline blaze::StaticMatrix<double, 3UL, 3UL> rotz(const double &theta)
{
    ...
    blaze::StaticMatrix<double, 3UL, 3UL> R{{c, -s, 0.00}, {s, c, 0.00}, {0.00, 0.00, 1.00}};

    return R;
}

/**
 * @brief Computes and returns the time derivative of the rotation matrix in SO(3) around the z-axis R_z for an angle
 * 'theta' in radians.
 *
 * @param theta The input angle in radians.
...
{
    double c(cos(theta)), s(sin(theta));
    blaze::StaticMatrix<double, 3UL, 3UL> dR{{-s, c, 0.00}, {-c, -s, 0.00}, {0.00, 0.00, 0.00}};

    return dR;
}

/**
 * @brief Computes and returns hat operator applied on a 3-dimensional vector. This operation maps a 3-dimensional
 * vector in R^3 into the corresponding element in the space of skew-symmetric matrices in so(3).
 *
...
{
    blaze::StaticMatrix<double, 3UL, 3UL> Res{
...
/**
 * @brief Computes and returns the pre-multiplication of a 3-dimensional square matrix by the hat operator applied on a
 * 3-dimensional vector. That is to say, the operation: \hat{v} * M
 *
 * @param v The input 3-dimensional static Blaze vector.
 ...
 */
inline blaze::StaticMatrix<double, 3UL, 3UL> hatPreMultiply(const blaze::StaticVector<double, 3UL> &v,
...
/**
 * @brief Computes and returns the pos-multiplication of a 3-dimensional square matrix by the hat operator applied on a
 * 3-dimensional vector. That is to say, the operation: M * \hat{v}
 *
 * @param v The input 3-dimensional static Blaze vector.
 * @param M The corresponding 3-dimensional square Blaze matrix to be premultiplied.
 * @result The 3-dimensional square Blaze matrix containing the result of the operation.
 */
inline blaze::StaticMatrix<double, 3UL, 3UL> hatPostMultiply(const blaze::StaticMatrix<double, 3UL, 3UL> &M,
                                                             const blaze::StaticVector<double, 3UL> &v)
{
    blaze::StaticMatrix<double, 3UL, 3UL> Res{
        {M(0UL, 1UL) * v[2UL] - M(0UL, 2UL) * v[1UL], -M(0UL, 0UL) * v[2UL] + M(0UL, 2UL) * v[0UL],
         M(0UL, 0UL) * v[1UL] - M(0UL, 1UL) * v[0UL]},
        {M(1UL, 1UL) * v[2UL] - M(1UL, 2UL) * v[1UL], -M(1UL, 0UL) * v[2UL] + M(1UL, 2UL) * v[0UL],
         M(1UL, 0UL) * v[1UL] - M(1UL, 1UL) * v[0UL]},
        {M(2UL, 1UL) * v[2UL] - M(2UL, 2UL) * v[1UL], -M(2UL, 0UL) * v[2UL] + M(2UL, 2UL) * v[0UL],
         M(2UL, 0UL) * v[1UL] - M(2UL, 1UL) * v[0UL]}};
 ...
 */
inline blaze::StaticVector<double, 3UL> transposePreMultiply(const blaze::StaticMatrix<double, 3UL, 3UL> &R,
                                                             const blaze::StaticVector<double, 3UL> &v)
{
 ...
 * @param h The input 4-dimensional square Blaze vector corresponding to the orientation of the local body frame of the
 * backbone.
 * @param The 4-dimensional static Blaze vector corresponding to the local rate of change of the orientation of the
 * backbone with respect to arc-length.
 */
inline blaze::StaticVector<double, 4UL> quaternionDiff(const blaze::StaticVector<double, 3UL> &u,
                                                       const blaze::StaticVector<double, 4UL> &h)
{
