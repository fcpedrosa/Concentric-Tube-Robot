...
int main()
{
    ...
    // ── Kinematic Jacobian (3×6, finite differences) ─────────────────────────
    {
        ...
        report("kinematic_jacobian", timeIt(
                                         [&]
                                         {
                                             const auto J = robot.kinematicJacobian(guess);
                                             doNotOptimize(J);
                                         },
                                         50UL, 5UL));
    }

    // ── IK: cold solve to a verified-reachable target ────────────────────────
    {
        // Target generated by FK at a reference configuration (guaranteed reachable).
        CTR probe = testing::makeReferenceRobot();
        blaze::StaticVector<double, 6UL> qRef = testing::kHomeConfig;
        qRef[4UL] = math::deg2Rad(60.0);
        qRef[5UL] = math::deg2Rad(-45.0);
        bvp_type gProbe{};
        std::ignore = probe.actuate(qRef, gProbe);
        const blaze::StaticVector<double, 3UL> target = probe.tipPosition();

        report("ik_cold_reachable", timeIt(
                                        [&]
                                        {
                                            CTR robot = testing::makeReferenceRobot();
                                            bvp_type guess{};
    ...
    // ── IK: worst case — unreachable target, full iteration budget ───────────
    {
        report("ik_unreachable_budget", timeIt(
                                            [&]
                                            {
                                                CTR robot = testing::makeReferenceRobot();
                                                bvp_type guess{};
                                                std::ignore = robot.actuate(testing::kHomeConfig, guess);
                                                // The historical demo target: outside the workspace.
